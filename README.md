...
Let's start understanding more the *SOAP*.
...
Another point is that `RESTful Web Services` are OK for public API like weather applications while 'SOAP is suitable for critical systems *(B2B)* like core banking.
...
* `REST APIs` are more convenient with JavaScript and can be implemented easily as well. `SOAP APIs` are also convenient with JavaScript but don't recommended for greater implementation;
